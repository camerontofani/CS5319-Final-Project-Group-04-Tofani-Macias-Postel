...
// screen a: split marketing + auth; toggle login vs sign up
export function LoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setMessage(null);
    try {
      // hits monolith /api/auth/signup (same stub works for demo)
      await postJson("/api/auth/signup", { email, password });
      navigate("/onboarding");
    } catch (err) {
      if (looksOffline(err)) {
        setMessage(
          "Could not reach the api (is the backend running on port 8000?). Use “Explore demo” below to skip.",
        );
      } else {
    ...
    <div className="login-split">
      ...
      <div className="login-panel">
        ...
        <p className="form-hint login-skip">
          ...
          <span className="muted"> — works without the backend running</span>
        </p>
