...
// screen b: 3-step wizard (courses, availability grid, preferences)
export function OnboardingPage() {
  ...
  // simple grid: key "d-h" -> selected or not
  ...
  async function generatePlan() {
    try {
      // sends full wizard payload to monolith plan service (stub)
      await postJson("/api/plans/generate", payloadPreview);
    } catch {
      // still go to dashboard so the ui works if the server is off
    }
    navigate("/study-plan");
  }
