...
// screen c: week strip + day timeline + task list + deadlines + ai button
export function StudyPlanPage() {
  ...
  async function requestAi() {
    setAiBusy(true);
    try {
      // hits monolith route that forwards to ai_service in the same process
      await postJson("/api/ai/recommend", {
        topic: "Upcoming exams",
      });
      alert("AI response logged (stub) — check network tab");
    } catch {
      alert(
        "Could not reach the api. Start the backend (port 8000) or use this screen as a static demo.",
      );
    } finally {
      setAiBusy(false);
    }
